...
<div class="container my-5">
    ...
    @else
        ...
        <div class="card mb-4 shadow-sm">
            <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center" onclick="toggleDropdown({{ $key }})" style="cursor: pointer;">
                <h5 class="mb-0">Pengaduan: {{ $report->created_at }}</h5>
                                        ...
                                        <ul class="list-group list-group-flush">
                                            @foreach ($progresses as $progress)
                                                @if (is_array($progress->histories) && count($progress->histories) > 0)
                                                    ...
                                                    @endforeach
                                                @else
                                                    <li class="list-group-item text-muted">Tidak ada progress untuk response ini.</li>
                                                @endif
                                            @endforeach
                                        </ul>
                                    </div>
